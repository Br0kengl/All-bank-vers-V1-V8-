...
class UserData:
    ...
    def load_from_file(filename="BankV2LoginHash.json"):
        if not os.path.exists(filename):
            initial_data = {
                "users": [
                    {"login": "admin", "password": "takethel", "group": "admin", "snowmoney": 0},
                    {"login": "Br0k3n_", "password": "lollollol", "group": "UwU", "snowmoney": 0},
                    {"login": "lol", "password": "lol", "group": "admin", "snowmoney": 0}
                ]
            }
...
class Bank(QDialog):
    def __init__(self, username, userdata, parent=None):
        super().__init__(parent)
    ...
    def delete_user(self):
        dialog = QDialog(self)
        dialog.setWindowTitle("Удаление пользователя")
        
        layout = QVBoxLayout()
        dialog.setLayout(layout)

        username_input = QLineEdit()
        btn_confirm = QPushButton("Удалить")

        layout.addWidget(QLabel("Логин пользователя:"))
        layout.addWidget(username_input)
        layout.addWidget(btn_confirm)
        
        def confirm():
            login_to_delete = username_input.text().strip()
            if not login_to_delete:
                QMessageBox.warning(dialog, "Ошибка", "Введите логин пользователя.")
                return

            if login_to_delete == self.username:
                QMessageBox.warning(dialog, "Ошибка", "Вы не можете удалить себя.")
                return

            if self.userdata.del_user(login_to_delete):
                QMessageBox.information(dialog, "Успех", f"Пользователь '{login_to_delete}' удален.")
                dialog.close()
            else:
                QMessageBox.warning(dialog, "Ошибка", f"Пользователь '{login_to_delete}' не найден.")
