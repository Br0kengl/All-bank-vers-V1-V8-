...
if os.path.exists("BankV1LoginHash.json.json"):
    with open("BankV1LoginHash.json.json", "r") as file:
        data = json.load(file)
        data = data["users"]
else:
	data = {"users":[{
	...
	with open("BankV1LoginHash.json.json", "w") as file:
		file.write(obj)
    ...
    def auth(self):
        ...
        for user in data:
            if user['login'] == user_login and user['password'] == user_pass:
                msg.setText(f"Вы вошли в аккаунт {user['group']}")
            else:
